...
- P2：时间线模块聚类与主故事线（`[timeline_modules]`）、Callback Function List（`callbacks.*`）、PS 队列/栈（`ps_info`，无段 skip）、堆 `leak_suspects` / `fragmentation`；规则 `callback_*` / `ps_queue_pressure` / `timeline_storyline` / `heap_fragmentation` / `leak_suspect`。
